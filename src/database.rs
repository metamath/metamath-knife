...
impl Database {
    ...
    pub fn statements_until(&self, label: Label) -> Option<impl Iterator<Item = StatementRef<'_>> + '_> {
        let token = self.name_result().atom_name(label);
        let lookup = self.name_result().lookup_label(token)?;
        Some(self.segments.segments().into_iter().flatten().take_while(move |sref| {
    ...
    pub fn cmp(&self, left: &Label, right: &Label) -> Option<std::cmp::Ordering> {
        ...
        Some(self.parse_result().order.cmp(&lookup_left.address, &lookup_right.address))
    }
